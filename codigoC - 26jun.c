...
void agregarCliente(struct FarmaSalud *farmacia) {
    ...
    while (1) {
        printf("Ingrese RUT del cliente (Sin puntos ni guion, pero debe incluir digito verificador): ");
        scanf("%s", rutCliente);
...
float calcularEdadPromedioClientes(struct FarmaSalud *farmaSalud) {
    ...
    rec = farmaSalud->clientes;

    // Verificar si hay al menos un cliente en la lista
    if (rec == NULL) {
        printf("\nNo existen clientes en el sistema.\n");
        return 0.0;
    }
...
void mostrarPorcentajeSucursalesConVentasBajoPromedio(struct FarmaSalud *farmaSalud) {
    ...
    printf("\nEl porcentaje de sucursales con ventas bajo el promedio es: %.2f%%\n", porcentaje);
}
...
float calcularPorcentajeClientesAfiliados(struct FarmaSalud *farmaSalud) {
    ...
    if (farmaSalud == NULL || farmaSalud->clientes == NULL) {
        printf("\nNo existen clientes en el sistema.\n");
        return 0.0;
    }
...
void mostrarSucursalConMasVentas(struct FarmaSalud *farmaSalud) {
    ...
    // Verificar si hay al menos una sucursal en la lista
    if (farmaSalud->sucursales == NULL) {
        cls();
        printf("\nNo existen sucursales en el sistema.\n");
        pause();
        return;
    ...
    if (!sucursalMasVentas) {
        cls();
        printf("\nHay más de una sucursal con la misma cantidad de ventas o no hay sucursales.\n");
        pause();
        return;
    }
